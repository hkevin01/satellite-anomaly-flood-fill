...
use floodfill_core::{find_all_components, Connectivity, FloodFillMetrics};
use features::{FeatureTracker, extract_grid_components, ThreatLevel, Feature};
use decisions::{decide, Policy, DecisionContext, ActionType, SubsystemType, SpacecraftMode};
...
enum AnomalyType {
    ...
    Growing { center_x: usize, center_y: usize, max_radius: usize },
    /// Linear propagation
    Linear { start_x: usize, start_y: usize, direction: (i32, i32), length: usize },
    /// Random scattered anomalies
    Scattered { count: usize, intensity_range: (u16, u16) },
    /// Cascading failure simulation
    Cascade { initial_points: Vec<(usize, usize)>, spread_rate: f64 },
}
...
impl Simulation {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let solar_grid = Grid2D::new(SIM_GRID_WIDTH, SIM_GRID_HEIGHT, Cell::default())?;
        let thermal_grid = Grid2D::new(SIM_GRID_WIDTH, SIM_GRID_HEIGHT, Cell::default())?;
        let sensor_grid = Grid2D::new(64, 64, Cell::default())?; // Smaller sensor array
        
        let feature_tracker = FeatureTracker::new(MAX_SIM_FEATURES);
        ...
        let scenarios = vec![
            AnomalyScenario {
                ...
                duration: 30.0,
                injection_type: AnomalyType::HotSpot { x: 32, y: 16, intensity: 500 },
            ...
            AnomalyScenario {
                ...
                duration: 60.0,
                injection_type: AnomalyType::Growing { center_x: 64, center_y: 32, max_radius: 15 },
            ...
            AnomalyScenario {
                ...
                duration: 45.0,
                injection_type: AnomalyType::Scattered { count: 25, intensity_range: (200, 400) },
            ...
            AnomalyScenario {
                ...
                injection_type: AnomalyType::Linear { 
                    start_x: 10, start_y: 10, direction: (1, 1), length: 20 
                },
                expected_threat: ThreatLevel::High,
        ...
        println!("✅ Simulation initialized with {} scenarios", scenarios.len());
    ...
    fn run_all_scenarios(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (num, success, duration) in results {
            ...
            println!("Scenario {}: {} ({:.2}s)", num, status, duration.as_secs_f64());
            if success { passed += 1; }
        }
        
        println!("Success rate: {}/{} ({:.1}%)", 
                 passed, self.scenarios.len(), 
    ...
    fn execute_scenario(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        while self.sim_time < scenario.duration {
            ...
            if self.metrics.total_cycles % 50 == 0 {
                ...
                print!("\r🔄 Progress: {}% | Anomalies: {} | Actions: {} | Time: {:.1}s", 
                       progress, self.metrics.total_anomalies, 
                       self.metrics.total_actions, self.sim_time);
                io::stdout().flush()?;
            }
    ...
    fn inject_anomalies(&mut self, scenario: &AnomalyScenario) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match &scenario.injection_type {
            ...
            AnomalyType::Growing { center_x, center_y, max_radius } => {
            ...
            AnomalyType::Linear { start_x, start_y, direction, length } => {
                let progress = (self.sim_time / scenario.duration).min(1.0);
            ...
            AnomalyType::Scattered { count, intensity_range } => {
                if self.sim_time.fract() < 0.1 { // Inject new anomalies occasionally
                    for _ in 0..*count {
                        ...
                        let intensity = intensity_range.0 + 
                            ((current_time as u16 * 31) % (intensity_range.1 - intensity_range.0));
            ...
            AnomalyType::Cascade { initial_points, spread_rate } => {
    ...
    fn update_component_metrics(&mut self, grid_name: &str, components: &[floodfill_core::RegionStats]) {
        let entry = self.metrics.component_metrics
        ...
        if !components.is_empty() {
            ...
            entry.max_size = entry.max_size.max(components.iter().map(|c| c.count).max().unwrap_or(0));
    ...
    fn update_decision_metrics(&mut self, action: &ActionType, decision_time: Duration) {
        ...
        *metrics.action_counts.entry(action_name.to_string()).or_insert(0) += 1;
        ...
        if metrics.avg_decision_time_us == 0 {  
            ...
            metrics.avg_decision_time_us = 
                (metrics.avg_decision_time_us + decision_time_us) / 2;
        }
    }
    
    /// Execute simulated action
    fn execute_simulated_action(&mut self, action: ActionType) -> Result<(), Box<dyn std::error::Error>> {
        match action {
            ActionType::NoAction => {},
            ActionType::Monitor { .. } => {
    ...
    fn update_context(&mut self) {
        ...
        let orbital_phase = (self.sim_time % orbital_period) / orbital_period * 2.0 * std::f64::consts::PI;
    ...
    fn validate_scenario_results(&self, scenario: &AnomalyScenario) -> Result<(), Box<dyn std::error::Error>> {
        let features = self.feature_tracker.features();
        
        // Check if we detected appropriate threat levels
        let max_threat = features.iter()
            .map(|f| f.threat_level)
            .max()
            .unwrap_or(ThreatLevel::None);
        
        if max_threat < scenario.expected_threat {
            println!("⚠️  Warning: Expected threat level {:?}, but maximum detected was {:?}",
                     scenario.expected_threat, max_threat);
        }
        
        // Check if appropriate actions were taken
        let action_taken = self.metrics.decision_metrics.action_counts
            .get(&scenario.expected_action)
            .unwrap_or(&0);
        
        if *action_taken == 0 {
            println!("⚠️  Warning: Expected action '{}' was not taken", scenario.expected_action);
        }
    ...
    fn display_metrics(&self) {
        ...
        println!("   Active features: {}", self.feature_tracker.features().len());
    ...
    fn display_detailed_metrics(&self) {
        ...
        println!("  Total anomalies detected: {}", self.metrics.total_anomalies);
        println!("  Total actions executed: {}", self.metrics.total_actions);
        println!("  Average cycle time: {:.2}μs", self.metrics.avg_cycle_time_us);
        ...
        println!("\nDecision Engine:");
        println!("  Decisions made: {}", self.metrics.decision_metrics.decisions_made);
        println!("  Emergency actions: {}", self.metrics.decision_metrics.emergency_actions);
        println!("  Average decision time: {:.2}μs", self.metrics.decision_metrics.avg_decision_time_us);
        
        if !self.metrics.decision_metrics.action_counts.is_empty() {
            println!("  Action distribution:");
            for (action, count) in &self.metrics.decision_metrics.action_counts {
                let percentage = (*count as f64 / self.metrics.decision_metrics.decisions_made as f64) * 100.0;
    ...
    fn run_benchmark(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let high_stress_scenario = AnomalyScenario {
            ...
            injection_type: AnomalyType::Scattered { count: 100, intensity_range: (300, 600) },
            expected_threat: ThreatLevel::High,
        ...
        println!("\n✅ Benchmark completed in {:.2}s", total_time.as_secs_f64());
        
        // Display benchmark results
        println!("\n🏆 Benchmark Results:");
        println!("   Throughput: {:.0} cycles/sec", 1000.0 / total_time.as_secs_f64());
        println!("   Average cycle time: {:.2}μs", self.metrics.avg_cycle_time_us);
        println!("   Peak anomalies: {}", self.metrics.total_anomalies);
        println!("   Components detected: {}", 
                 self.metrics.component_metrics.values().map(|m| m.detected).sum::<u64>());
        println!("   Actions executed: {}", self.metrics.total_actions);
    ...
    fn run_custom_scenario(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let custom_scenario = AnomalyScenario {
            ...
            description: format!("User-defined hot spot at ({}, {}) with intensity {}", x, y, intensity),
            duration: 30.0,
